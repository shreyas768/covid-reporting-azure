...
df_raw_population = spark.read.csv("/mnt/covidreportingshreyas7dl/raw/population", sep=r'\t', header=True)
...
df_dim_country = spark.read.csv("/mnt/covidreportingshreyas7dl/lookup/country_lookup/country_lookup.csv", sep=r',', header=True)
...
df_processed_population.write.format("com.databricks.spark.csv").option("header","true").option("delimiter", ",").mode("overwrite").save("/mnt/covidreportingshreyas7dl/processed/population")
